...
app.get('/api/messages/:retailer_name', (req, res) => {
  try {
    const db = readDB();
    const { retailer_name } = req.params;
    if (retailer_name === 'all') {
      res.json(db.messages);
    } else {
      const messages = db.messages.filter(m => m.retailer_name === retailer_name);
      res.json(messages);
    }
  } catch (error) {
    res.status(500).json({ error: error.message });
...
app.post('/api/messages', (req, res) => {
  try {
    const db = readDB();
    const { retailer_name, sender, text, timestamp } = req.body;
    const newMessage = {
      id: db.messages.length > 0 ? Math.max(...db.messages.map(m => m.id)) + 1 : 1,
      retailer_name, sender, text, timestamp, is_read: false
    };
...
app.put('/api/messages/read/:retailer_name', (req, res) => {
  try {
    const db = readDB();
    const { retailer_name } = req.params;
    const { reader } = req.body; // 'admin' or 'retailer'
    
    let updated = false;
    db.messages.forEach(m => {
      if (m.retailer_name === retailer_name && m.sender !== reader && !m.is_read) {
        m.is_read = true;
        updated = true;
      }
    });
